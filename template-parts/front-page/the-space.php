 ...
 * Displays six image gallery cards, a heading, and a social CTA card.
 * Each card image is fetched from WordPress media by attachment ID.
 * Cards without an image are not rendered.
 *
...
// Build image cards from customizer (6 cards).
$space_cards = [];
for ( $i = 1; $i <= 6; $i++ ) {
    $img_id   = absint( get_theme_mod( "goldenpine_space_card{$i}_image", 0 ) );
    $img_url  = $img_id ? wp_get_attachment_image_url( $img_id, 'large' ) : '';
    $title    = get_theme_mod( "goldenpine_space_card{$i}_title", '' );
    ...
    $is_gif = strtolower( pathinfo( $img_url, PATHINFO_EXTENSION ) ) === 'gif';
...
<section id="space" class="relative py-24 md:py-32 px-6 lg:px-12 bg-background overflow-hidden">
    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="grid grid-cols-1 md:grid-cols-12 gap-6 md:gap-10 items-end mb-14 md:mb-16">
            <?php if ( $_gpine_space_heading_1 || $_gpine_space_heading_2 ) : ?>
        ...
        <!-- Social CTA Card -->
        <?php if ( $_gpine_space_cta_label || $_gpine_space_cta_text || $_gpine_space_instagram || $_gpine_space_facebook ) : ?>
            <div class="rounded-3xl border border-border bg-card p-8 md:p-10 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
                ...
                <!-- CTA copy -->
                <?php if ( $_gpine_space_cta_label || $_gpine_space_cta_text ) : ?>
                    <div>
                        <?php if ( $_gpine_space_cta_label ) : ?>
                ...
                <!-- Social buttons -->
                <?php if ( $_gpine_space_instagram || $_gpine_space_facebook ) : ?>
