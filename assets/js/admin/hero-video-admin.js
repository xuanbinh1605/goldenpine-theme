 ...
 * video attachments for the Hero Videos meta boxes (PC & Mobile) on the gpine_video CPT.
 *
 * Dependencies:
...
( function ( $, wp, l10n ) {
    ...
    function initGallery( device ) {
        var $list  = $( '#gpine-video-list-' + device );
        var $input = $( '#gpine_hero_videos_' + device );
        var $empty = $( '.gpine-video-gallery[data-device="' + device + '"] .gpine-video-gallery__empty' );
        var frame;

        if ( ! $list.length || ! $input.length ) {
            return; // Gallery not found on page.
        }
        ...
        // ------------------------------------------------------------------
        $( '#gpine-add-videos-' + device ).on( 'click', function () {
            ...
            frame = wp.media( {
                title:    l10n.frameTitle + ' (' + device.toUpperCase() + ')',
